let common = {
    ...
    search_do: (act) => {
        ...
        // Get the selected value for searching
        if (act === 'users') {
            selectedField = document.querySelector('#search-field').value;
        } else {
            selectedField = 'plots';
        }
        let data = { search: gv('search'), selectedField: selectedField };
    ...
    user_edit_update: (user_id = 0) => {
        ...
        const phoneValue = gv('phone');
        const phoneNumberPattern = /^\d+$/;

        if (!phoneNumberPattern.test(phoneValue)) {
            alert("The field 'Phone' should only contain numbers.");
            return false;
        }
        ...
        for (const field of fields) {
            if (!gv(field)) {
                alert(`The field '${field.replace('_', ' ')}' is empty.`);
                return false;
            }
        }
